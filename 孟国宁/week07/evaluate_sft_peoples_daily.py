...
教学重点：
  1. 生成式 NER 的评估方式：JSON 解析 → span-level F1（与 llm_ner_peoples_daily.py 一致）
  2. LoRA adapter 自动识别：目录含 adapter_config.json → LoRA，否则 → 全量
  3. 与 BERT+CRF 的对比：生成式 vs 序列标注，各有什么优劣

使用方式：
  python evaluate_sft_peoples_daily.py                              # 评估 LoRA 模型（默认）
  python evaluate_sft_peoples_daily.py --ckpt_dir ../outputs/sft_full_ckpt_peoples_daily  # 评估全量微调模型
  python evaluate_sft_peoples_daily.py --n_samples 50 --demo        # 5 条示例快速演示
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT 人民日报 NER 评估")
    ...
    parser.add_argument("--n_samples",   default=100, type=int,
                        help="验证集采样数（与 llm_ner_peoples_daily.py 默认 100 条对齐）")
...
def main():
    ...
    if not ckpt_dir.exists():
        ...
        print("请先运行 train_sft_peoples_daily.py 完成训练。")
        print("  LoRA（默认）保存到:   outputs/sft_adapter_peoples_daily/")
        print("  全量微调保存到:        outputs/sft_full_ckpt_peoples_daily/")
        return
    ...
    llm_zero_f1   = "?"
    llm_few_f1    = "?"
    crf_log_path  = LOG_DIR / "eval_crf_validation_peoples_daily.json"
...
评估标准说明：
  SFT（本脚本）和 LLM API（llm_ner_peoples_daily.py）均使用 span F1：
    text.find() 近似定位 → (text, type, start, end) 4元组 → 与 gold 做集合交集
  BERT+CRF（evaluate_peoples_daily.py）使用 seqeval：
