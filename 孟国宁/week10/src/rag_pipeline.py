...
使用方式：
  ...
  python rag_pipeline.py --query "Transformer的注意力机制是什么"
  python rag_pipeline.py --query "..." --query-rewrite
  python rag_pipeline.py --query "..." --source-file "my_paper.pdf"

依赖：
  pip install faiss-cpu rank_bm25 jieba openai numpy python-dotenv
...
EMBED_DIM       = 1024
LLM_MODEL       = "qwen-plus"        # 可换 qwen-turbo / qwen-max
...
3. 引用具体内容时，在句末标注来源编号，如：注意力机制的核心公式为... [1]
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "你是检索查询优化专家。将用户的问题改写为更适合从学术文献中检索信息的精确查询语句。"
                    "保留关键实体（论文名、作者、技术术语、年份），扩展相关关键词，不要超过50字。"
                    "直接输出改写后的查询语句，不要解释。"
                ),
            },
...
class RAGPipeline:
    ...
    def query(
        ...
        if top_score < SCORE_THRESHOLD and filter_meta is None:
            return {
                "answer": "根据文献知识库未能找到与该问题相关的内容，建议查阅原始文献。",
...
def main():
    parser = argparse.ArgumentParser(description="文献 RAG 问答（原生版）")
    ...
    parser.add_argument("--source-file",   type=str,  default=None, help="限定文献来源文件")
    ...
    filter_meta = {}
    if args.source_file:
        filter_meta["source_file"] = args.source_file
    if not filter_meta:
        filter_meta = None
