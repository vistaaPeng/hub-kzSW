...
from src.weather_backend import get_coordinates, get_weather_by_coords  # noqa: E402
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "根据城市中文名获取经纬度信息，返回包含纬度和经度的结果。用于后续调用 get_weather_by_coords 查询天气。",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "根据经纬度查询当前天气及未来3天预报。必须先调用 get_coordinates 获取目标城市的经纬度，再使用返回的纬度和经度调用本工具。",
            "parameters": {
                "type": "object",
                "properties": {
                    "lat": {"type": "number", "description": "纬度，例如 26.64"},
                    "lon": {"type": "number", "description": "经度，例如 119.31"},
                },
                "required": ["lat", "lon"],
            },
        },
...
SYSTEM_PROMPT = (
    ...
    "涉及天气时，必须先调用 get_coordinates 获取城市的经纬度，然后使用返回的纬度和经度调用 get_weather_by_coords 查询天气。"
    "本回合你可以一次调用多个工具，需要时会进行多轮工具调用。"
)


def run(client, model: str, question: str, verbose: bool = True) -> dict:
    """
    链式闭环：提问 → 模型输出 tool_call → 执行 → 回填 → 循环直到无工具调用 → 最终回答。
