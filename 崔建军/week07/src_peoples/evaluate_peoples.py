"""
在测试集上评估 BERT NER 模型（peoples_daily 数据集），并统计非法序列

与 cluener 评估脚本的功能完全一致，但使用 peoples_daily 数据集：
  - 3 类实体（PER/ORG/LOC）
  - 数据格式为分词 token 列表 + BIO 标签列表
...
使用方式：
  python evaluate_peoples.py                        # 评估 BERT+Linear
  python evaluate_peoples.py --use_crf              # 评估 BERT+CRF
  python evaluate_peoples.py --split validation     # 在验证集上评估
"""
...
def main():
    ...
    run_tag = "crf_peoples" if args.use_crf else "linear_peoples"
    ckpt_path = CKPT_DIR / f"best_{run_tag}.pt"

    if not ckpt_path.exists():
        print(f"找不到 checkpoint：{ckpt_path}")
        print(f"请先运行：python train_peoples.py {'--use_crf' if args.use_crf else ''}")
        return
    ...
    result_path = LOG_DIR / f"eval_{run_tag}_{split_name}.json"
...
def parse_args():
    parser = argparse.ArgumentParser(description="评估 BERT NER 模型（peoples_daily 数据集）")
