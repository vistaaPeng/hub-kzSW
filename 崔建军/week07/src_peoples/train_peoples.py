...
与 cluener 训练脚本的功能完全一致，但使用 peoples_daily 数据集：
  - 3 类实体（PER/ORG/LOC）
...
使用方式：
  python train_peoples.py                        # 训练 BERT+Linear（基线）
  python train_peoples.py --use_crf             # 训练 BERT+CRF
  python train_peoples.py --epochs 5 --lr 3e-5 # 自定义超参数
...
def main():
    ...
    print(f"\n开始训练（{'BERT+CRF' if args.use_crf else 'BERT+Linear'}）— peoples_daily 数据集...")
    ...
    print(f"\n下一步：python evaluate_peoples.py {'--use_crf' if args.use_crf else ''}")
