...
教学重点：
  1. 文本匹配数据的结构——sentence pair + binary label
  2. 类别分布分析——正负样本比例
  3. 句子长度分布——为BERT max_length选择提供依据
  4. 正/负样本的长度差异——检测是否存在length bias
  5. Token 数 vs 字符数——BERT tokenizer的分词粒度
...
依赖：
  pip install matplotlib transformers jieba
"""
...
def plot_label_distribution(splits_data, output_dir):
    ...
    fig.suptitle("LCQMC 标签分布", fontproperties=fp, fontsize=12, y=1.02)
    fig.tight_layout()
...
def plot_word_overlap(rows, output_dir):
    """分析正负样本的词汇重叠度"""
    print("  计算词汇重叠度（使用jieba分词）...")
    ...
    pos_overlaps = [calc_overlap(r["sentence1"], r["sentence2"])
                    for r in rows if r["label"] == 1]
    neg_overlaps = [calc_overlap(r["sentence1"], r["sentence2"])
                    for r in rows if r["label"] == 0]

    fp = _get_font()
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.hist(pos_overlaps, bins=30, alpha=0.6, label=f"正样本 均值={np.mean(pos_overlaps):.3f}",
            color="#2196F3", density=True)
    ax.hist(neg_overlaps, bins=30, alpha=0.6, label=f"负样本 均值={np.mean(neg_overlaps):.3f}",
            color="#F44336", density=True)
    ax.set_xlabel("词汇重叠度 (Jaccard)", fontproperties=fp)
    ax.set_ylabel("密度", fontproperties=fp)
    ax.set_title("正/负样本词汇重叠度分布", fontproperties=fp)
    ax.legend(prop=fp)
    fig.tight_layout()

    save_path = output_dir / "word_overlap_distribution.png"
    fig.savefig(save_path, dpi=150)
    plt.close(fig)
    print(f"  图表已保存 → {save_path}")
    print(f"  词汇重叠度：正样本均值={np.mean(pos_overlaps):.3f}  负样本均值={np.mean(neg_overlaps):.3f}")
...
def parse_args():
    ...
    parser.add_argument("--skip_overlap", action="store_true", help="跳过词汇重叠度分析（较慢）")
...
def main():
    ...
    if not args.skip_overlap:
        plot_word_overlap(train_rows, args.output_dir)
