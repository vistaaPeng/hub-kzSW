...
教学重点：
  1. PairDataset — 句对数据集，用于CosineEmbeddingLoss训练和评估
  2. TripletDataset — 三元组数据集，用于TripletLoss训练（对比学习）
  3. CrossEncoderDataset — 交互型数据集，句对拼接为单序列送入BERT
  4. TraditionalDataset — 传统方法数据集，用于编辑距离、词向量、TF-IDF、BM25

使用方式：
  from dataset import PairDataset, TripletDataset, CrossEncoderDataset, TraditionalDataset
...
class TraditionalDataset(Dataset):
    """
    传统方法数据集：用于编辑距离、词向量、TF-IDF、BM25等传统方法

    不需要tokenizer，直接返回原始文本和标签
...
def build_traditional_loaders(data_dir, batch_size=32):
    """为传统方法构建 DataLoader"""
    data_dir = Path(data_dir)
    train_ds = TraditionalDataset(data_dir / "train.jsonl")
    val_ds   = TraditionalDataset(data_dir / "validation.jsonl")
    test_ds  = TraditionalDataset(data_dir / "test.jsonl")

    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=False, num_workers=0)
    val_loader   = DataLoader(val_ds,   batch_size=batch_size, shuffle=False, num_workers=0)
    test_loader  = DataLoader(test_ds,  batch_size=batch_size, shuffle=False, num_workers=0)

    print(f"  train : {len(train_ds):>7,} 条")
    print(f"  val   : {len(val_ds):>7,} 条")
    print(f"  test  : {len(test_ds):>7,} 条")
    return train_loader, val_loader, test_loader