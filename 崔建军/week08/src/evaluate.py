...
教学重点：
  1. BiEncoder评估的关键差异——输出是相似度分数，需要阈值搜索才能变为预测标签
  2. 阈值搜索(threshold search)：在验证集上枚举阈值，选F1最高的
  3. CrossEncoder评估与分类任务完全一样——直接取argmax即为预测标签
  4. 传统方法评估——同样需要阈值搜索
  5. 相似度分布图——正负样本在相似度轴上的分离程度是模型质量的直观反映
...
def eval_traditional(model, data_loader, find_threshold=True, threshold=0.5):
    """
    传统方法评估：计算每对句子的相似度，然后搜索最优阈值。
    ...
    for batch in data_loader:
        ...
        # 批量计算相似度
        sentence_pairs = [(s1, s2) for s1, s2 in zip(s1_list, s2_list)]
        sims = model.predict(sentence_pairs)
...
def parse_args():
    ...
    parser.add_argument("--model_type", required=True,
                        choices=["biencoder", "crossencoder", "traditional"])
    parser.add_argument("--ckpt",       type=str, help="checkpoint .pt 文件路径")
    ...
    parser.add_argument("--traditional_method", default="edit_distance",
                        choices=["edit_distance", "word_vector", "tfidf", "bm25"])
...
def main():
    ...
    if args.model_type == "biencoder":
        ...
        # 构建模型
        if args.traditional_method == "edit_distance":
            model = build_edit_distance_model()
            method_name = "编辑距离"
        elif args.traditional_method == "word_vector":
            model = build_word_vector_model()
            method_name = "词向量"
        elif args.traditional_method == "tfidf":
            # 需要先在训练集上训练
            ...
            method_name = "TF-IDF"
        elif args.traditional_method == "bm25":
            # 需要先在训练集上训练
            train_path = Path(args.data_dir) / "train.jsonl"
            train_ds = TraditionalDataset(train_path)
            train_sentences = []
        ...
        print(f"\n{'='*50}")
        print(f"{method_name} 评估结果（{args.split}，{len(ds)} 条）")
        print(f"  最优阈值: {metrics['threshold']:.2f}")
        print(f"  Accuracy: {metrics['accuracy']:.4f}")
        print(f"  F1      : {metrics['f1']:.4f}")
        ...
        plot_similarity_distribution(
            metrics["similarities"], metrics["labels"], metrics["threshold"],
            save_path=FIG_DIR / f"{args.traditional_method}_{args.split}_sim_dist.png",
            title=f"{method_name} 相似度分布（{args.split}）",
        )
