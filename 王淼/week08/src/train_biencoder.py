...
# ── 训练一个 epoch ─────
def train_one_epoch_cosine(model,loader,optimizer,scheduler,device,epoch,
                           total_epochs,margin,grad_accum):
    model.train()
    ...
    for step,batch in enumerate(pbar):
        ...
        # CosineEmbeddingLoss 需要将 label 转为 -1/1
...
def train_one_epoch_triplet(model, loader, optimizer, scheduler, device,
    ...
    for step, batch in enumerate(pbar):
        enc_a = {
            "input_ids":      batch["input_ids_anchor"].to(device),
            "attention_mask": batch["attention_mask_anchor"].to(device),
            "token_type_ids": batch["token_type_ids_anchor"].to(device),
        }
        enc_p = {
            "input_ids":      batch["input_ids_positive"].to(device),
            "attention_mask": batch["attention_mask_positive"].to(device),
            "token_type_ids": batch["token_type_ids_positive"].to(device),
        }
        enc_n = {
            "input_ids":      batch["input_ids_negative"].to(device),
            "attention_mask": batch["attention_mask_negative"].to(device),
            "token_type_ids": batch["token_type_ids_negative"].to(device),
        }
...
def main():
    ...
    if args.loss == "cosine":
        ...
        train_loader, val_loader = build_triplet_loader(
            data_dir, tokenizer, 
            max_length=args.max_length, batch_size=args.batch_size)
        test_loader = None  # TripletLoss 训练不使用 test 集评估
    ...
    # ── 分层学习率 ───
    # BERT 骨干和池化层共享一个学习率，通常较小；新加的 dropout 层学习率可稍大
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.loss == "cosine":
            train_loss = train_one_epoch_cosine(
                ...
                epoch, args.epochs, margin=args.margin, grad_accum=args.grad_accum)
        else:  # triplet
        ...
        print(f"Epoch {epoch}/{args.epochs} - "
              ...
              f"Val Acc: {val_acc:.4f}  Val F1: {val_f1:.4f}  Val Thr: {val_thr:.4f}  "
              f"Time: {elapsed:.1f}s")
        ...
        if val_f1 > best_val_f1:
            ...
            print(f"  新最佳模型已保存: {ckpt_path} (Val F1: {val_f1:.4f})")
...
def parse_args():
    ...
    parser.add_argument("--dataset",           default="bq_corpus",
