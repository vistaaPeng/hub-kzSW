...
def load_and_eval(method, tokenizer, device, dataset, split, batch_size):
    ...
    if method["type"] == "biencoder":
        ...
        data_path = DATA_DIR / dataset / f"{split}.jsonl"
...
def main():
    ...
    for m in METHODS:
        print(f"\n{'='*55}")
        print(f"加载 {m['key']} ...")
        metrics = load_and_eval(m, tokenizer, device, args.dataset, args.split, args.batch_size)
        if metrics is None:
            continue
