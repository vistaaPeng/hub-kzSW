...
def gold_spans_from_record(record: dict) -> set[tuple[str, str, int, int]]:
    ...
    peoples_daily 数据格式：{"tokens": [...], "ner_tags": ["O", "B-PER", "I-PER", ...]}
    需要将 BIO 标签转为 span 格式。
    """
    spans = set()
    ...
    while i < n:
        ...
        if tag.startswith("B-"):
            ...
            while j < n and ner_tags[j] == f"I-{etype}":
                j += 1
            # span: tokens[i:j] 是实体文本，start=i, end=j-1
...
FEW_SHOT_EXAMPLES = [
    ...
    {
        "text": "北京大学是一所知名的高等院校。",
        "output": '{"entities": [{"text": "北京大学", "type": "ORG"}]}'
    },
]
