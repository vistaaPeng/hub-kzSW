...
教学重点：
  ...
  2. 非法序列统计：量化 CRF vs 线性头的关键差异
     - 非法：I-X 在序列开头，或 B-X 后面接 I-Y（X≠Y）
     - 线性头通常有几十到几百条非法序列，CRF 始终为 0
  3. 逐类的 F1 分析：哪类实体最难识别？
...
def count_illegal_sequences(pred_seqs: list[list[str]]) -> dict:
    ...
    非法类型：
      ...
      - illegal_transition：B-X 或 I-X 后面接 I-Y（X≠Y）
    """
...
def main():
    ...
    if args.use_crf:
        if illegal_stats["total_illegal"] == 0:
            print("  -> CRF Viterbi 解码：非法序列 0 条 OK（转移矩阵已充分学习约束）")
        else:
