"""
NER 数据集类：peoples_daily 的 BIO 标注格式 + BERT 子词对齐

教学重点：
  1. peoples_daily 的 BIO 格式直接是字符级标签
     - tokens: 字符列表
     - ner_tags: BIO 标签列表（O, B-PER, I-PER, B-ORG, I-ORG, B-LOC, I-LOC）
...
def build_dataloaders(
    ...
    if num_train > 0:
        import random
        random.seed(42)
        train_records = random.sample(train_records, min(num_train, len(train_records)))
    if num_val > 0:
        val_records = val_records[:num_val]
