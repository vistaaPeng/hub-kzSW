...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "根据经纬度查询天气。必须先调用 geocode_city 获取经纬度，再用此工具查询天气。",
            "parameters": {
                "type": "object",
                "properties": {
...
SYSTEM_PROMPT = (
    ...
    "查询天气需要两步：首先调用 geocode_city(city='城市名') 获取经纬度，然后调用 get_weather_by_coords(latitude, longitude) 查询天气。"
    "本回合你可以一次调用多个工具。"
)
...
def run(client, model: str, question: str, verbose: bool = True) -> dict:
    """
    多轮循环闭环：提问 → 模型输出 tool_call → 执行 → 回填 → 模型决定是否继续调用工具 → 最终回答。
    ...
    elapsed = time.time() - t0
    if verbose:
        print(f"  → [llm] 达到最大轮数，返回当前回答（{elapsed:.1f}s）")
    return {"answer": answer, "tool_calls": tool_call_log, "elapsed": elapsed}
