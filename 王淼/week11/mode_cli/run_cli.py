...
NAMED_TOOLS_SCHEMA = [
    {
        ...
        "function": {
            "name": "rag_search",
            "description": "在A股年报语料库中检索与问题最相关的段落。",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "检索问题，不要包含公司名和年份，用简短财务术语"},
                    "stock_code": {"type": "string", "description": "可选，股票代码，如 '300750'"},
                    "year": {"type": "string", "description": "可选，年份：'2021' / '2022' / '2023'"},
                    "top_k": {"type": "integer", "description": "返回段落数，默认5"},
                },
                "required": ["query"],
            },
        },
    ...
    },
    {
        ...
        "function": {
            "name": "geocode",
            "description": "根据城市名称查询经纬度。返回 JSON 格式，包含城市名称、经纬度、国家和省级行政区信息。查天气前必须先调用此工具获取经纬度。",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "根据经纬度查询天气。必须先调用 geocode 获取经纬度，再用此工具查询天气。",
            "parameters": {
                "type": "object",
                "properties": {
                    "latitude": {"type": "number", "description": "纬度，如 26.64"},
                    "longitude": {"type": "number", "description": "经度，如 119.52"},
                },
...
# 形态 → (schema, executor)
MODE_DISPATCH = {
    "named": (NAMED_TOOLS_SCHEMA, lambda args, name: run_named(name, args)),
    "bash": (BASH_TOOLS_SCHEMA, lambda args, name: run_bash(args.get("command", ""))),
}
...
SYSTEM_PROMPT_NAMED = (
    ...
    "查询天气需要两步：首先 run_cli(command='geocode', args={'city':'城市名'}) 获取经纬度，然后 run_cli(command='weather_by_coords', args={'latitude':纬度, 'longitude':经度}) 查询天气。"
...
SYSTEM_PROMPT_BASH = (
    ...
    "查询天气需要两步：首先 fincli geocode --city 宁德 获取经纬度，然后 fincli weather-by-coords --latitude 纬度 --longitude 经度 查询天气。"
...
def run(client, model: str, question: str, mode: str, verbose: bool = True) -> dict:
    ...
    for round_num in range(max_rounds):
        ...
        for tc in msg.tool_calls:
            ...
            if verbose:
                print(f"  → [{mode}] {name}({args})")
            try:
                result = executor(args, name)
