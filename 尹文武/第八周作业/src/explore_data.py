...
BERT_PATH  = ROOT.parent.parent / "pretrained_models" / "bert-base-chinese"
    ...
    for ax, (split_name, rows) in zip(axes, splits_data.items()):
        ...
        for bar, c in zip(bars, counts):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 20,
                    f"{c}\n({c/len(rows)*100:.1f}%)", ha="center", va="bottom", fontsize=9)
        ax.set_title(f"{split_name}（{len(rows):,} 条）")
        ax.set_ylabel("数量")
        ax.tick_params(axis="x", labelsize=9)

    fig.suptitle("AFQMC 标签分布（正例约 31%，负例约 69%）", fontsize=12, y=1.02)
    fig.tight_layout()
