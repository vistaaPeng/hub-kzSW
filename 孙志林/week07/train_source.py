"""
在人民日报数据集上训练 BERT NER 模型（第七周作业）

复用原项目 src/model.py 中的 BertNER 和 BertCRFNER，
新实现人民日报数据集的加载（dataset_peoples_daily.py）。
...
# 复用原项目的模型代码
_SRC_DIR = Path(__file__).parent.parent / "week7 序列标注问题" / "序列标注项目" / "src"
...
BERT_PATH = ROOT.parent.parent / "week4语言模型(1)" / "bert-base-chinese"
...
def main():
    ...
    print(f"\n开始训练（{'BERT+CRF' if args.use_crf else 'BERT+Linear'}，数据集：人民日报）...")
...
def parse_args():
    parser = argparse.ArgumentParser(description="在人民日报数据集上训练 BERT NER 模型")
