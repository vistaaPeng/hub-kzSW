"""
在测试集上评估 BERT NER 模型，并统计非法序列 - 人民日报数据集

与 cluener 版本的主要区别：
  - 使用人民日报数据集（peoples_daily）
  - 标签体系：O / B-PER / I-PER / B-ORG / I-ORG / B-LOC / I-LOC（7类）
  - 评估集：使用 validation.json（test.json 标签未公开）

核心逻辑保持不变：
  - seqeval entity-level 评估
  - 非法序列统计（量化 CRF vs 线性头）
  - 逐类型 F1 分析
...
# BERT 模型路径：优先本地路径，不存在则自动从 HuggingFace 下载
BERT_PATH = ROOT / "bert-base-chinese"
if not BERT_PATH.exists():
    BERT_PATH = "bert-base-chinese"  # HuggingFace 模型ID，会自动下载
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(str(args.bert_path))
