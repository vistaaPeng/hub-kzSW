...
def _load_bert(bert_path: str) -> BertModel:
    ...
    # 读取本地的 bert_config.json
    config_path = os.path.join(bert_path, "bert_config.json")
    with open(config_path, 'r', encoding='utf-8') as f:
        local_config = json.load(f)
    
    # 强制设置 vocab_size 为 21128（中文 BERT 标准大小）
    local_config['vocab_size'] = 21128
    
    # 创建配置对象
    config = BertConfig.from_dict(local_config)
    
    # 加载模型，传入修正后的配置
    bert = BertModel.from_pretrained(bert_path, config=config)
