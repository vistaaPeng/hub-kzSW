...
# 随机生成一个5维向量，最大值认为是正样本，反之为负样本
def build_sample():
    x = np.random.random(5)   # 生成5维随机向量
    y = np.argmax(x)  # 找到最大值的索引作为类别（0-4）
...
def evaluate(model):
    ...
    with torch.no_grad():
        y_pred = model(x)  # 模型预测，输出logits
        _, predicted = torch.max(y_pred, 1)  # 取最大值索引作为预测类别
...
def predict(model_path, input_vec):
    ...
    with torch.no_grad():  # 不计算梯度
        ...
        _, predicted_classes = torch.max(result, 1)  # 预测的类别
    for vec, pred_class, probs in zip(input_vec,  predicted_classes, probabilities):
        print("输入：%s, 预测类别：%d, 概率值：%s" % (vec, pred_class.item(), probs.numpy()))  # 打印结果
