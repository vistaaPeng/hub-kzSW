...
def get_weather(city: str | None = None, lat: float | None = None, lon: float | None = None) -> str:
    """
    查询指定位置的当前天气及未来3天预报。

    Args:
        city: 城市名称，支持中文，例如 "宁德"、"北京"、"上海"。若提供 lat/lon，则忽略此参数。
        lat: 纬度，例如 26.64
        lon: 经度，例如 119.52
    ...
    with httpx.Client(timeout=10.0) as client:
        ...
        if lat is not None and lon is not None:
            ...
            if not city:
                return "错误：必须提供 city 参数或 lat/lon 参数"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lat", type=float, help="纬度，如 26.64")
    parser.add_argument("--lon", type=float, help="经度，如 119.52")
    args = parser.parse_args()
    print(get_weather(city=args.city, lat=args.lat, lon=args.lon))
