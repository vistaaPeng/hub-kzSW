...
from common.config import DATA_DIR, OUTPUT_DIR, QWEN_MODEL_NAME, LABEL_NAMES, ZERO_SHOT_DEFAULTS
...
def classify_one(text, model, tokenizer, device, max_new_tokens=8):
    ...
    # transformers 4.x: apply_chat_template(tokenize=True) 返回 list，直接给 tensor
...
def main():
    ...
    for i, item in enumerate(samples):
        ...
        results.append({"text": item["sentence"], "true": true_name, "pred": pred_name, "correct": is_correct})

    elapsed = time.time() - t0
    acc = correct / total
    print(f"\nZero-shot 结果: acc={acc:.4f} ({correct}/{total})  unparseable={unparseable}  耗时={elapsed:.0f}s")

    out_path = OUTPUT_DIR / "llm_zero_shot_results.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"accuracy": acc, "total": total, "correct": correct, "unparseable": unparseable, "results": results}, f, ensure_ascii=False, indent=2)
    print(f"结果 → {out_path}")
