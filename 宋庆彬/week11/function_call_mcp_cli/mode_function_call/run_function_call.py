...
教学重点：
  ...
  2. 循环闭环：模型输出 tool_call → 宿主执行工具 → 结果以 role=tool 回填
     → 继续请求模型，直到模型返回普通答案
...
与其它方式的关系：
  本文件的 LLM 工具调用循环，和 mode_mcp/run_mcp.py、mode_cli/run_cli.py 几乎一样，
...
def run(client, model: str, question: str, verbose: bool = True) -> dict:
    """
    循环闭环：提问 → 模型输出 tool_call → 执行 → 回填，直到生成最终回答。
