...
# ── （LangChain 版本项目不实现，故此处无 run_langchain_rag）───────────────────
...
def run_ragas_eval(results: list[dict], pipeline_name: str) -> dict:
    ...
    # 配置 RAGAS 统一使用 DashScope（打分 LLM 与 embedding 同一个 key）
...
def main():
    parser = argparse.ArgumentParser(description="标准/法律 RAG 系统 RAGAS 评估（仅原生版）")
    parser.add_argument("--pipeline",     choices=["native"], default="native")
