...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM zero-shot LCQMC 文本匹配评估")
    ...
    parser.add_argument("--num_samples", default=200, type=int,
                        help="评估样本数（默认 200，全集太贵；LCQMC val 条目较多）")
...
def main():
    ...
    print(f"\n{'='*55}")
    print(f"LLM 评估结果（{args.model}，{len(samples)} 条样本，LCQMC）")
