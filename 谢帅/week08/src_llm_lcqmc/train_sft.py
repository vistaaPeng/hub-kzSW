"""
LLM SFT（监督微调）训练脚本 — 基于 LoRA 高效微调 Qwen2.5-0.5B-Instruct 做 LCQMC 文本匹配
...
使用方式：
  python train_sft.py                        # LoRA，50000 条快速演示（默认）
  python train_sft.py --num_train -1         # LoRA，全部条目
...
MODEL_PATH = Path("E:/my/LLM/课程/pretrain_models/Qwen2.5-0.5B-Instruct")
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT LCQMC 文本匹配训练（LoRA / 全量微调）")
...
def main():
    ...
    # ── 正负平衡采样（LCQMC 正负比不平衡，需平衡采样避免模型全预测负例）──────
    ...
    print(f"\n下一步：python evaluate_sft.py 查看 Accuracy / F1")
