"""
加载 SFT checkpoint（LoRA / 全量微调），在 LCQMC 验证集上评估文本匹配 Accuracy / F1
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT LCQMC 文本匹配评估")
...
def main():
    ...
    # ── SFT 结果输出 ──────────────────────────────────────────────────────────
    print(f"\n{'='*65}")
    print(f"LLM SFT LCQMC 文本匹配评估结果")
    print(f"{'='*65}")
