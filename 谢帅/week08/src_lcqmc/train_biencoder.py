"""
BiEncoder 训练脚本（LCQMC 表示型文本匹配，Sentence-BERT 架构）
...
使用方式：
  ...
  # 自定义参数
  python train_biencoder.py --loss cosine --pool mean --num_hidden_layers 12 --epochs 3
...
def parse_args():
    parser = argparse.ArgumentParser(description="BiEncoder 训练（LCQMC 表示型文本匹配）")
    ...
    parser.add_argument("--num_hidden_layers", default=12,    type=int,
                        help="BERT Transformer 层数（默认 12 层全量；可设 4 层快速验证）")
