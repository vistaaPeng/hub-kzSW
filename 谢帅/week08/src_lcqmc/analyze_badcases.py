...
【1】数据层面
  ...
  │
  ├─ 数据增强（正样本扩充）
  │    LCQMC 正样本约 10K 条（~45%），TripletLoss 三元组因此受限。
  │    → 对正样本做同义改写（换词、调序），扩充正例数量。
  │      可用 LLM API 批量生成改写句。
  │
  └─ 跨数据集迁移
       AFQMC / BQ Corpus（已下载）包含更多样的句对。
       → 先在 LCQMC（238K 对）上预训练 Sentence-BERT，再 fine-tune 到其他领域，
         利用大数据集的语义泛化能力。
""")
...
""")
  ...
  │
  └─ 换用更好的预训练模型
       MacBERT / RoBERTa-Chinese 在通用语料上有更好的初始化，
       LCQMC 中很多错误源于口语化问句的语义理解不准确。
""")
...
""")
  ...
  │    这是 RAG Reranker 的完整版。
  │    → 可用当前两个 checkpoint 直接组合，无需重新训练。
  │
  └─ 训练更多 epoch + 全量 12 层
       当前使用 12 层 × 3 epoch 训练，完整训练预计进一步提升。
       → 建议实验：不同 epoch 数和层数的消融对比。
""")
