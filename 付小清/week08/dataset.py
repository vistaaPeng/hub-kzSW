"""
work8 文本匹配数据集（LCQMC / BQ Corpus）

不依赖上级 src/，数据来自 ../data/{lcqmc,bq_corpus}/
"""
...
def resolve_data_dir(dataset: str) -> Path:
    if dataset not in SUPPORTED_DATASETS:
        raise ValueError(f"dataset 须为 {SUPPORTED_DATASETS}，收到: {dataset}")
...
class TripletDataset(Dataset):
    ...
    def _build_triplets(self, rows):
        ...
        for r in rows:
            if r["label"] != 1:
                continue
            anchor, positive = r["sentence1"], r["sentence2"]
