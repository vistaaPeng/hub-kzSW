"""
在人民日报 NER 上评估 BERT 序列标注模型

使用方式：
  cd work7
  python evaluate.py                  # BERT + Linear，验证集
  python evaluate.py --use_crf        # BERT + CRF
  python evaluate.py --split test     # 测试集（有标签，可算 F1）
"""
...
def main():
    ...
    print(f"\n正在在 [{args.split}] 集上推理（peoples_daily）...")
    ...
    print("\n" + "=" * 70)
    print(f"模型：{'BERT + CRF' if args.use_crf else 'BERT + Linear'}  |  数据集：peoples_daily  |  {args.split}")
    print("=" * 70)
...
def parse_args():
    parser = argparse.ArgumentParser(description="人民日报 NER 评估")
    parser.add_argument("--use_crf", action="store_true")
    parser.add_argument("--bert_path", type=Path, default=BERT_PATH)
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--split", choices=["validation", "test"], default="test",
                        help="peoples_daily 测试集有标签，默认在 test 上评估")
