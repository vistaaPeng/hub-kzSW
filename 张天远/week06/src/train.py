...
def parse_args():
    ...
    parser.add_argument("--loss_type",       default="plain",
                        choices=["plain", "balanced", "soft", "focal"],
                        help="loss 类型: plain(普通), balanced(加权), soft(平方根平滑加权), focal(FocalLoss)")
    parser.add_argument("--focal_gamma",     default=2.0, type=float,
                        help="Focal Loss 的 gamma 参数")
    parser.add_argument("--freeze_bert",     action="store_true",
                        help="冻结 BERT 层，只训练分类头（两阶段训练）")
...
class FocalLoss(nn.Module):
    """Focal Loss：对难分样本（pt 低）自动放大 loss，适合不均衡数据。"""
...
def build_criterion(loss_type, data_dir, num_labels, device, focal_gamma=2.0):
    """工厂函数：根据 loss_type 返回对应的 loss 函数。"""
    if loss_type == "plain":
        print("使用普通 CrossEntropyLoss")
        return nn.CrossEntropyLoss()

    if loss_type in ("balanced", "soft"):
        with open(data_dir / "train.json", encoding="utf-8") as f:
            train_data = json.load(f)
        labels = np.array([item["label"] for item in train_data])
        classes = np.arange(num_labels)
        ...
        if loss_type == "soft":
            ...
            print("使用 Soft 加权 CrossEntropyLoss（平方根平滑）")
        else:
            print("使用加权 CrossEntropyLoss（inverse-frequency）")
        # 打印权重
        with open(data_dir / "label_map.json", encoding="utf-8") as f:
            id2name = {int(k): v for k, v in json.load(f)["id2name"].items()}
        for i, w in enumerate(weights):
...
def main():
    ...
    # ── Loss ────────────────────────────────────────────────────────────────
    criterion = build_criterion(args.loss_type, data_dir, num_labels, device, args.focal_gamma)

    # ── 冻结 BERT（两阶段训练） ─────────────────────────────────────────────
    if args.freeze_bert:
        for param in model.bert.parameters():
            param.requires_grad = False
        print("BERT 层已冻结，只训练分类头")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            run_tag = f"{args.pool}_{args.loss_type}"
            if args.freeze_bert:
                run_tag += "_freeze"
