...
def parse_args():
    ...
    parser.add_argument("--biencoder_cosine_ckpt",  default=None, type=str,
                        help="BiEncoder Cosine checkpoint 路径（默认: biencoder_cosine_best.pt）")
    parser.add_argument("--biencoder_triplet_ckpt", default=None, type=str,
                        help="BiEncoder Triplet checkpoint 路径（默认: biencoder_triplet_best.pt）")
    parser.add_argument("--crossencoder_ckpt",      default=None, type=str,
                        help="CrossEncoder checkpoint 路径（默认: crossencoder_best.pt）")
...
def main():
    ...
    # 允许命令行覆盖 checkpoint 路径（支持消融实验的自定义命名）
    _ckpt_overrides = {
        "biencoder_cosine":  args.biencoder_cosine_ckpt,
        "biencoder_triplet": args.biencoder_triplet_ckpt,
        "crossencoder":      args.crossencoder_ckpt,
    }
    for m in METHODS:
        if _ckpt_overrides.get(m["key"]):
            m["ckpt"] = _ckpt_overrides[m["key"]]
