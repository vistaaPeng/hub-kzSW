...
def train_one_epoch_cosine(model, loader, optimizer, scheduler, device,
    ...
    for step, batch in enumerate(pbar):
        ...
        if args.loss == "infonce":
            # InfoNCE: batch 内对比学习，所有 s2 互为负例
            temperature = getattr(args, 'temperature', 0.05)
            sim = (emb_a @ emb_b.T) / temperature  # [B, B]
            targets = torch.arange(sim.size(0), device=device)
            # 仅正例行参与 loss 计算（负例作为 distractor）
            pos_mask = (labels == 1)
            if pos_mask.sum() > 1:
                loss = F.cross_entropy(sim[pos_mask], targets[pos_mask])
            else:
                continue
        else:
...
def train_one_epoch_triplet(model, loader, optimizer, scheduler, device,
                            epoch, total_epochs, margin, grad_accum, hard_neg="random"):
    """TripletLoss 训练 loop。hard_neg='online' 时使用 in-batch 在线难负例。"""
    model.train()
    total_loss, total_samples = 0.0, 0
    optimizer.zero_grad()

    pbar = tqdm(loader, desc=f"Epoch {epoch}/{total_epochs} [Triplet{'Online' if hard_neg=='online' else ''}]", leave=False)
    for step, batch in enumerate(pbar):
        enc_a = {
            "input_ids":      batch["input_ids_a"].to(device),
            "attention_mask": batch["attention_mask_a"].to(device),
        ...
        if hard_neg == "online":
            # 在线难负例：在当前 batch 内找余弦相似度最高的非正例样本
            # 拼接所有样本 [3B, H]
            all_emb = torch.cat([emb_a, emb_p, emb_n], dim=0)
            # 余弦相似度矩阵 [B, 3B]（已 L2 归一化，点积 = 余弦相似度）
            sim = emb_a @ all_emb.T
            B = emb_a.size(0)
            # 排除 anchor 自身 (col i) 和正例 (col B+i)
            for i in range(B):
                sim[i, i] = -2.0
                sim[i, B + i] = -2.0
            hard_idx = sim.argmax(dim=1)  # [B]
            emb_n_hard = all_emb[hard_idx]
            loss = F.triplet_margin_loss(emb_a, emb_p, emb_n_hard, margin=margin)
        else:
...
def main():
    ...
    # ── 从 checkpoint 加载权重（混合训练 / 迁移学习）─────────────────────
    if args.resume_from:
        resume_path = Path(args.resume_from)
        if not resume_path.exists():
            print(f"❌ resume_from checkpoint 不存在: {resume_path}")
            return
        print(f"从 {resume_path.name} 加载预训练权重...")
        resume_ckpt = torch.load(resume_path, map_location=device, weights_only=False)
        model.load_state_dict(resume_ckpt["state_dict"])
        print(f"  源模型 val_f1={resume_ckpt.get('val_f1', 'N/A')}  epoch={resume_ckpt.get('epoch', 'N/A')}")
        print(f"  继续训练 {args.epochs} epoch（新数据）...")
    ...
    # ── 训练循环 ──────────────────────────────────────────────────────────
    # checkpoint 命名规则：biencoder_{loss}_{pool}_L{layers}_best.pt
    # 默认配置 (pool=mean, layers=4) 简化为 biencoder_{loss}_best.pt
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.loss == "cosine":
            ...
            train_loss = train_one_epoch_triplet(
                model, train_loader, optimizer, scheduler, device,
                epoch, args.epochs, args.margin, args.grad_accum,
                hard_neg=getattr(args, 'hard_neg', 'random'),
            )
    ...
    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / f"biencoder_{args.loss}{_pool_tag}{_layer_tag}_log.json"
...
def parse_args():
    ...
    parser.add_argument("--loss",              default="cosine",
                        choices=["cosine", "triplet", "infonce"],
                        help="cosine / triplet / infonce（batch内对比学习）")
    parser.add_argument("--temperature",       default=0.05, type=float,
                        help="InfoNCE 温度参数（仅 --loss infonce）")
    ...
    parser.add_argument("--resume_from",       default=None, type=str,
                        help="从 checkpoint 加载权重继续训练（混合训练/迁移学习）")
    parser.add_argument("--hard_neg",          default="random", choices=["random", "online"],
                        help="TripletLoss 负例策略：random=离线随机, online=in-batch在线难负例")
