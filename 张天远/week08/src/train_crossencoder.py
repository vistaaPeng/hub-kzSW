...
def main():
    ...
    # ── 训练循环 ──────────────────────────────────────────────────────────
    # checkpoint 命名规则：crossencoder_L{layers}_best.pt
    # 默认配置 (layers=4) 简化为 crossencoder_best.pt
    _layer_tag = f"_L{args.num_hidden_layers}" if args.num_hidden_layers != 4 else ""
    ckpt_path   = CKPT_DIR / f"crossencoder{_layer_tag}_best.pt"
    best_val_f1 = 0.0
    log_records = []
    ...
    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / f"crossencoder{_layer_tag}_log.json"
