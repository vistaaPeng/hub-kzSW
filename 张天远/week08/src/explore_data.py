...
def plot_label_distribution(splits_data, output_dir):
    fp = _get_font()
    fig, axes = plt.subplots(1, len(splits_data), figsize=(5 * len(splits_data), 3.5))
    ...
    for ax, (split_name, rows) in zip(axes, splits_data.items()):
        ...
        for bar, c in zip(bars, counts):
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + max(counts) * 0.02,
                    ...
                    fontsize=9)
        ax.set_title(f"{split_name}（{len(rows):,} 条）", fontsize=10)
        ax.set_ylabel("数量")

    fig.subplots_adjust(top=0.85)
    fig.suptitle("AFQMC 标签分布", fontsize=12)
    save_path = output_dir / "label_distribution.png"
    fig.savefig(save_path, dpi=120, bbox_inches=None)
    plt.close(fig)
...
def main():
    ...
    # 预先加载中文字体（必须在前，plt.subplots() 之前生效）
    _get_font()
