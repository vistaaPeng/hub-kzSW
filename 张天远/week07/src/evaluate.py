...
def main():
    ...
    # 模型短名（与 train.py 保持一致的命名规则）
    model_path_str = args.bert_path.as_posix().lower()
    if "roberta" in model_path_str:
        model_tag = "roberta"
    elif "bert" in model_path_str:
        model_tag = ""
    else:
        model_tag = args.bert_path.name[:15]
    ...
    labels, label2id, id2label = build_label_schema(dataset=args.dataset)

    model = build_model(
        use_crf=args.use_crf,
        bert_path=args.bert_path.as_posix(),
        num_labels=len(labels),
    ).to(device)
    model.load_state_dict(ckpt["state_dict"])
    print(f"加载 checkpoint（epoch={ckpt['epoch']}，val_f1={ckpt['val_entity_f1']:.4f}）")

    tokenizer = BertTokenizer.from_pretrained(args.bert_path.as_posix())
    ...
    print(f"\n正在在 [{args.dataset}/{split_name}] 集上推理...")
    ...
    print("\n" + "=" * 70)
    print(f"模型：{args.dataset} + {'CRF' if args.use_crf else 'Linear'}  |  评估集：{split_name}")
    print("=" * 70)
    ...
    result = {
        "model": f"{args.dataset}+{'CRF' if args.use_crf else 'Linear'}",
        "dataset": args.dataset,
        "split": split_name,
        "precision": round(p, 6),
        "recall": round(r, 6),
    ...
    # 逐样本预测保存（--save_predictions）
    if args.save_predictions:
        pred_path = LOG_DIR / f"eval_{run_tag}_{split_name}_predictions.json"
        details = []
        for i, (gold, pred) in enumerate(zip(all_golds, all_preds)):
            illegal_flags = []
            for j in range(len(pred)):
                tag = pred[j]
                if j == 0 and tag.startswith("I-"):
                    illegal_flags.append({"pos": j, "type": "illegal_start", "tag": tag})
                elif j > 0 and tag.startswith("I-"):
                    prev = pred[j-1]
                    if prev == "O":
                        illegal_flags.append({"pos": j, "type": "illegal_transition", "detail": f"O -> {tag}"})
                    elif prev.startswith("B-") or prev.startswith("I-"):
                        if prev[2:] != tag[2:]:
                            illegal_flags.append({"pos": j, "type": "illegal_transition", "detail": f"{prev} -> {tag}"})
            details.append({
                "idx": i,
                "gold": gold,
                "pred": pred,
                "illegal_flags": illegal_flags,
                "has_illegal": len(illegal_flags) > 0,
            })
        with open(pred_path, "w", encoding="utf-8") as fout:
            json.dump(details, fout, ensure_ascii=False, indent=2)
        illegal_count = sum(1 for d in details if d["has_illegal"])
        print(f"逐样本预测已保存 → {pred_path}（{illegal_count} 条含非法序列）")
