...
def _load_bert(bert_path: str):
    # Monkey-patch 绕过 transformers 的 torch>=2.6 检查
    # (hfl/chinese-roberta-wwm-ext 只有 .bin 格式)
    # 必须补到 modeling_utils 上——import_utils 的函数已被 from-import 本地引用
...
def build_model(
    ...
    model_name = f"{bert_path.rsplit('/', 1)[-1]} + {'CRF' if use_crf else 'Linear'}"
