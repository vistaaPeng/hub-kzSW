...
class SFTDataset(Dataset):
    ...
    def __init__(self, data, tokenizer, max_length=256, dataset="cluener2020"):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        prompt_text = self.tokenizer.apply_chat_template(
            [
                {"role": "system", "content": build_system_prompt(self.dataset)},
...
def main():
    ...
    data_dir   = Path(args.data_dir) if args.data_dir else (
        ROOT / "data" / ("cluener" if args.dataset == "cluener2020" else args.dataset))
    ...
    if Path(args.model_path).exists():
        model_src = str(Path(args.model_path).resolve())
    ...
    # ── 构建数据集 ─────────────────────────────────────────────────────────────
    train_dataset = SFTDataset(train_raw, tokenizer, args.max_length, dataset=args.dataset)
    val_dataset   = SFTDataset(val_raw[:300], tokenizer, args.max_length, dataset=args.dataset)
