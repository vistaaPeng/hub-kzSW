...
SYSTEM_PROMPT = (
    ...
    "涉及天气时调用 get_weather。你可以分多轮调用工具，先查一个城市再查另一个对比。"
)


def run(client, model: str, question: str, verbose: bool = True) -> dict:
    """
    ReAct 多轮闭环：提问 → 模型输出 tool_call → 执行 → 回填 → 可继续调工具 → ... → 最终回答。
    最多 5 轮，模型不调工具或达上限即止。返回 {answer, tool_calls, elapsed} 用于对比器汇总。
    """
    messages = [
    ...
    # ── ReAct 多轮循环 ──────────────────────────────────────────────────
    # 模型可以多轮调用工具：调工具 → 看到结果 → 继续调 → ... → 直到不再调或达上限
    MAX_TURNS = 5
    turn = 0
    msg = None
...
DEMO_QUESTIONS = [
    ...
    "宁德和北京今天天气对比，哪个更热？",  # ReAct 多轮：先查宁德再查北京
]
