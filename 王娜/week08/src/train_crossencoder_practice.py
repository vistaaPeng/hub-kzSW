"""
CrossEncoder 训练脚本（LCQMC 练习版，交互型文本匹配）
...
使用方式：
  ...
  python train_crossencoder_practice.py

  # 自定义参数
  python train_crossencoder_practice.py --num_hidden_layers 6 --epochs 5 --batch_size 16
...
from dataset_practice import build_crossencoder_loaders
from evaluate_practice import eval_crossencoder
...
def main():
    ...
    # LCQMC 正负比约 1:1，CrossEntropyLoss 默认不加权
    ...
    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / "crossencoder_practice_log.json"
    with open(log_path, "w", encoding="utf-8") as f:
        json.dump(log_records, f, ensure_ascii=False, indent=2)
    print(f"\n训练完成。最优 val_f1={best_val_f1:.4f}")
    print(f"训练日志 → {log_path}")
    print(f"最优 checkpoint → {ckpt_path}")
    print(f"\n运行评估：python evaluate_practice.py --model_type crossencoder --ckpt {ckpt_path}")


# ── 参数解析 ──────────────────────────────────────────────────────────────

def parse_args():
    parser = argparse.ArgumentParser(description="CrossEncoder 训练（LCQMC 练习版，交互型文本匹配）")
