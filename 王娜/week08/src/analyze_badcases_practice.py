...
使用方式：
  ...
  python analyze_badcases_practice.py

  # 分析 CrossEncoder
  python analyze_badcases_practice.py --model_type crossencoder \
    --ckpt ../outputs/checkpoints/crossencoder_practice_best.pt

  # 分析更多 bad case
  python analyze_badcases_practice.py --n_cases 20
...
from dataset_practice import PairDataset, CrossEncoderDataset, load_jsonl
from model_practice import build_biencoder, build_crossencoder
...
def plot_score_dist_with_errors(results, threshold, save_path, model_label):
    ...
    # 左图：正/负样本分数分布（同 evaluate_practice.py，基准参考）
    ax = axes[0]
...
【1】数据层面
  ...
  │
  ├─ 数据增强（正样本扩充）
  │    LCQMC 虽已有 238K 条，但口语化问句的表述多样性仍有限。
  │    → 对正样本做同义改写（换词、调序），扩充正例数量。
  │      可用 LLM API 批量生成改写句。
  │
  └─ 跨数据集迁移
       AFQMC / BQ Corpus（已下载）包含金融/客服领域的问句对。
       → 可尝试多任务预训练或在 BQ Corpus 上预训练后再微调 LCQMC，
         增强模型对不同领域问句的泛化能力。
""")
  ...
  │
  └─ 引入 Interaction 特征辅助
       BiEncoder 看不到词序交互，而这类 FP 往往需要理解"打篮球"vs"看篮球"
...
""")
  ...
  │
  └─ 换用更优的中文预训练模型
       MacBERT / RoBERTa-Chinese / Chinese-RoBERTa-wwm 在通用中文语料上表现更好，
       LCQMC 中很多错误源于对口语化表达的理解不准确。
""")
...
""")
  ...
  │
  └─ 训练更多 epoch + 全量 12 层
       本次演示用 4 层 × 1 epoch 快速验证，完整训练预计提升若干 F1 点。
...
def parse_args():
    parser = argparse.ArgumentParser(description="Bad Case 分析与优化方向（LCQMC 练习版）")
    parser.add_argument("--model_type", default="biencoder",
                        choices=["biencoder", "crossencoder"])
    parser.add_argument("--ckpt", default=None, type=str,
                        help="checkpoint 路径（默认自动选 biencoder_cosine_practice_best.pt）")
...
def main():
    ...
    if args.ckpt:
        ...
        ckpt_path = CKPT_DIR / "biencoder_cosine_practice_best.pt"
    else:
        ckpt_path = CKPT_DIR / "crossencoder_practice_best.pt"

    if not ckpt_path.exists():
        print(f"[ERR] checkpoint 不存在: {ckpt_path}")
        print("请先运行训练脚本（train_biencoder_practice.py / train_crossencoder_practice.py）")
        return
    ...
    # ── 可视化 ────────────────────────────────────────────────────────────
    fig_path = FIG_DIR / f"{args.model_type}_practice_badcase_dist.png"
