...
使用方式：
  from dataset_practice import PairDataset, TripletDataset, CrossEncoderDataset, build_pair_loaders
...
class TripletDataset(Dataset):
    ...
    构建逻辑（离线静态构建）：
      ...
      → 最终 triplet 数量 ≈ 正样本对数量（LCQMC 约 119K 条）
...
def build_pair_loaders(data_dir, tokenizer, max_length=64, batch_size=32):
    ...
    注意：LCQMC 三个 split 均带有标签，可直接用于评估。
    """
