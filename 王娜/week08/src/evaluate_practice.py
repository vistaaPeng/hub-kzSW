"""
文本匹配评估工具（LCQMC 练习版，可作模块导入，也可独立运行）
...
使用方式：
  ...
  python evaluate_practice.py --model_type biencoder --ckpt ../outputs/checkpoints/biencoder_cosine_practice_best.pt
  python evaluate_practice.py --model_type crossencoder --ckpt ../outputs/checkpoints/crossencoder_practice_best.pt

  # 作为模块导入（训练脚本内调用）
  from evaluate_practice import eval_biencoder, eval_crossencoder
...
def parse_args():
    parser = argparse.ArgumentParser(description="文本匹配模型评估（LCQMC 练习版）")
    ...
    parser.add_argument("--split",      default="validation", choices=["validation", "test"],
                        help="评估的数据集分割（LCQMC 三个 split 均含标签）")
...
def main():
    ...
    if args.model_type == "biencoder":
        ...
        from dataset_practice import PairDataset
        ...
        plot_similarity_distribution(
            ...
            save_path=FIG_DIR / f"biencoder_{args.split}_practice_sim_dist.png",
        ...
        from model_practice import build_crossencoder
        from dataset_practice import CrossEncoderDataset
        from torch.utils.data import DataLoader
