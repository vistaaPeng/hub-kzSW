...
使用方式：
  python compare_methods_practice.py
  python compare_methods_practice.py --split validation --batch_size 64
...
from dataset_practice import PairDataset, CrossEncoderDataset
from evaluate_practice import eval_biencoder, eval_crossencoder, plot_similarity_distribution
...
METHODS = [
    {
        ...
        "label":     "BiEncoder\n(CosineEmbeddingLoss)",
        "ckpt":      "biencoder_cosine_practice_best.pt",
        "type":      "biencoder",
        "color":     "#2196F3",
    },
    {
        ...
        "label":     "BiEncoder\n(TripletLoss)",
        "ckpt":      "biencoder_triplet_practice_best.pt",
        "type":      "biencoder",
        "color":     "#4CAF50",
    },
    {
        "key":       "crossencoder",
        "label":     "CrossEncoder\n(CrossEntropyLoss)",
        "ckpt":      "crossencoder_practice_best.pt",
        "type":      "crossencoder",
        "color":     "#FF9800",
    },
]
...
def parse_args():
    parser = argparse.ArgumentParser(description="三种文本匹配方法效果对比（LCQMC 练习版）")
...
def main():
    ...
    if not all_results:
        print("没有可用的 checkpoint，请先运行训练脚本（train_biencoder_practice.py / train_crossencoder_practice.py）。")
        return
    ...
    if len(bi_results) == 2:
        ...
        if abs(delta_f1) < 0.01:
            print("    → 两种 Loss 差距不大（1 epoch + 随机负采样限制了 Triplet 的优势）")
        elif delta_f1 > 0:
    ...
    log = [{k: _to_py(v) for k, v in m.items() if k not in SKIP_KEYS}
           for m in all_results]
    log_path = LOG_DIR / "method_comparison_practice.json"
    ...
    # ── 可视化 ────────────────────────────────────────────────────────────
    plot_comparison_bar(all_results, FIG_DIR / "method_comparison_practice_bar.png")

    bi_with_sim = [m for m in all_results if m["type"] == "biencoder" and "similarities" in m]
    if bi_with_sim:
        plot_sim_distributions(bi_with_sim, FIG_DIR / "biencoder_practice_sim_distributions.png")
