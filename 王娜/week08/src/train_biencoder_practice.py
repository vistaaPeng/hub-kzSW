"""
BiEncoder 训练脚本（LCQMC 练习版，表示型文本匹配，Sentence-BERT 架构）
...
使用方式：
  ...
  python train_biencoder_practice.py

  # TripletLoss
  python train_biencoder_practice.py --loss triplet

  # 自定义参数
  python train_biencoder_practice.py --loss cosine --pool mean --num_hidden_layers 4 --epochs 3
...
from dataset_practice import build_pair_loaders, build_triplet_loader
from evaluate_practice import eval_biencoder
from model_practice import build_biencoder
...
def main():
    ...
    # ── 检查 BERT 路径，本地不存在则使用 HuggingFace 在线模型 ─────────────────
    local_bert_path = Path(args.bert_path)
    # 检查目录是否存在且包含模型文件
    model_files = ["pytorch_model.bin", "model.safetensors"]
    has_model_file = local_bert_path.exists() and any(
        (local_bert_path / f).exists() for f in model_files
    )
    if has_model_file:
        bert_path = str(local_bert_path)
        print(f"使用本地 BERT: {bert_path}")
    else:
        bert_path = "bert-base-chinese"
        print(f"本地模型不存在，使用 HuggingFace 在线模型: {bert_path}")
    ...
    # ── 训练循环 ──────────────────────────────────────────────────────────
    ckpt_name = f"biencoder_{args.loss}_practice_best.pt"
    ...
    # ── 训练完成，保存日志 ────────────────────────────────────────────────
    log_path = LOG_DIR / f"biencoder_{args.loss}_practice_log.json"
    with open(log_path, "w", encoding="utf-8") as f:
        json.dump(log_records, f, ensure_ascii=False, indent=2)
    print(f"\n训练完成。最优 val_f1={best_val_f1:.4f}")
    print(f"训练日志 → {log_path}")
    print(f"最优 checkpoint → {ckpt_path}")
    print(f"\n运行评估：python evaluate_practice.py --model_type biencoder --ckpt {ckpt_path}")


# ── 参数解析 ──────────────────────────────────────────────────────────────

def parse_args():
    parser = argparse.ArgumentParser(description="BiEncoder 训练（LCQMC 练习版，表示型文本匹配）")
