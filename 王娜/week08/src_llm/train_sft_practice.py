"""
LLM SFT（监督微调）训练脚本（LCQMC 练习版）— 基于 LoRA 高效微调 Qwen2-0.5B-Instruct 做文本匹配
...
使用方式：
  python train_sft_practice.py                        # LoRA，5000 条快速演示（默认）
  python train_sft_practice.py --num_train -1         # LoRA，全部 238K 条
  python train_sft_practice.py --epochs 1             # 快速验证流程

  # 全量微调（需显存 ≥ 16GB）
  python train_sft_practice.py --full_ft --lr 2e-5
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT 文本匹配训练（LCQMC 练习版，LoRA / 全量微调）")
...
def main():
    ...
    ckpt_dir   = output_dir / ("sft_full_ckpt_practice" if args.full_ft else "sft_adapter_practice")
    ...
    # ── 正负平衡采样（LCQMC train 正负比约 58:42，不平衡会导致模型偏向多数类）──────
    ...
    log_path = log_dir / f"train_{log_tag}_practice.json"
    ...
    print(f"\n下一步：python evaluate_sft_practice.py 查看 Accuracy / F1 与多方对比")
