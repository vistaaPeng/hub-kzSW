...
与 BiEncoder / CrossEncoder / LLM API 多方对比（LCQMC 练习版）
...
使用方式：
  python evaluate_sft_practice.py                              # 评估 LoRA（默认）
  python evaluate_sft_practice.py --ckpt_dir ../outputs/sft_full_ckpt_practice  # 评估全量微调
  python evaluate_sft_practice.py --demo                       # 5 条示例快速演示
...
ADAPTER_DIR = ROOT / "outputs" / "sft_adapter_practice"
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM SFT 文本匹配评估（LCQMC 练习版）")
...
def main():
    ...
    if not ckpt_dir.exists():
        ...
        print("  LoRA（默认）保存到:   outputs/sft_adapter_practice/")
        print("  全量微调保存到:        outputs/sft_full_ckpt_practice/")
        return
    ...
    bi_cosine_f1  = read_log_f1(LOG_DIR / "biencoder_cosine_practice_log.json")
    bi_triplet_f1 = read_log_f1(LOG_DIR / "biencoder_triplet_practice_log.json")
    cross_f1      = read_log_f1(LOG_DIR / "crossencoder_practice_log.json")

    llm_api_acc = "?"
    llm_api_f1  = "?"
    llm_log     = LOG_DIR / "llm_compare_results_practice.json"
    if llm_log.exists():
...
""")
    ...
    # ── 保存结果 ──────────────────────────────────────────────────────────────
    out_path = LOG_DIR / "sft_results_practice.json"
