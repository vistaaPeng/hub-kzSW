"""
LLM API 文本匹配对比（LCQMC 练习版，DashScope）
...
使用方式：
  ...
  python llm_compare_practice.py
  python llm_compare_practice.py --num_samples 50 --model qwen-plus
...
def parse_args():
    parser = argparse.ArgumentParser(description="LLM zero-shot 文本匹配评估（LCQMC 练习版）")
    parser.add_argument("--data_dir",    default=str(DATA_DIR), type=str)
    parser.add_argument("--split",       default="validation",
                        choices=["validation", "test"])
    parser.add_argument("--num_samples", default=100, type=int,
                        help="评估样本数（默认 100，全集太贵；LCQMC val 共 8802 条）")
...
def main():
    ...
    # 保存结果到文件，供 evaluate_sft_practice.py 读取比较
    log_dir = ROOT / "outputs" / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    out_path = log_dir / "llm_compare_results_practice.json"
    save_data = {
        "model": args.model,
