...
app = FastAPI(
    title       = "学术文献 RAG 问答服务",
    description = "FAISS + BM25 混合检索 + DeepSeek API（DSA/半导体缺陷检测方向），含教学调试接口",
    version     = "1.0.0",
    lifespan    = lifespan,
)
...
class QueryRequest(BaseModel):
    question:    str           = Field(...,  example="DSA光刻技术的主要挑战是什么？")
    stock_code:  Optional[str] = Field(None, example="600519")
    year:        Optional[str] = Field(None, example="2023")
    source_file: Optional[str] = Field(None, example="导向自组装光刻工艺及器件制备研究", description="按来源文献名过滤")
...
def _build_source(item: dict) -> str:
    ...
    # 去除 .json 后缀 + _semantic 后缀
    source = source_file.replace("_semantic.json", "").replace(".json", "")
    if not source:
        source = item.get("stock_code", "") or "文献"
    s = source
...
def _filter_meta(req: QueryRequest) -> Optional[dict]:
    ...
    # 注意：block_types 在 metadata 中是列表，用支持列表过滤的检索引擎（如 ChromaDB）时生效
    return fm or None
...
def query_debug(req: QueryRequest):
    ...
    try:
        ...
        # ⑤ LLM 生成
        answer = _module.call_llm(req.question, context, pipeline.llm_client)
