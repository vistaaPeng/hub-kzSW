"""
peoples_daily NER 训练脚本（课后练习）

教学重点：
  1. BIO 格式数据的直接读取（无需 span→BIO 转换）
...
使用方式：
  python src/train_practice.py                        # 训练 BERT+Linear（基线）
  python src/train_practice.py --use_crf              # 训练 BERT+CRF
  python src/train_practice.py --epochs 5 --lr 3e-5  # 自定义超参数
