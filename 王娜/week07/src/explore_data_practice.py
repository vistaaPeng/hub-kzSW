...
输出：
  outputs/figures/entity_distribution_peoples_daily.png
  outputs/figures/text_length_distribution_peoples_daily.png
  outputs/figures/entity_length_distribution_peoples_daily.png
"""
...
def extract_entities_from_bio(tokens: list[str], ner_tags: list[str]) -> list[dict]:
    """从 BIO 标签序列中提取实体列表。"""
    entities = []
    i = 0
    n = len(ner_tags)
...
def collect_stats(records: list) -> dict:
    ...
    for row in records:
        ...
        for ent in entities:
            ...
            entities_by_type[etype].append(ent["surface"])
...
def print_summary(stats_train: dict, stats_val: dict):
    ...
    print(f"  平均实体数/句：{sum(stats_val['entity_per_sentence']) / len(stats_val['entity_per_sentence']):.2f}")
...
def plot_entity_distribution(stats_train: dict):
    ...
    fig.savefig(FIG_DIR / "entity_distribution_peoples_daily.png", dpi=120)
    print(f"  已保存 → {FIG_DIR / 'entity_distribution_peoples_daily.png'}")
    plt.close()
...
def plot_text_length_distribution(stats_train: dict):
    ...
    ax.set_xlabel("文本字符数（token 数）")
    ax.set_ylabel("样本数")
    ax.legend()
    plt.tight_layout()
    fig.savefig(FIG_DIR / "text_length_distribution_peoples_daily.png", dpi=120)
    print(f"  已保存 → {FIG_DIR / 'text_length_distribution_peoples_daily.png'}")
    plt.close()
...
def plot_entity_length_distribution(stats_train: dict):
    ...
    ax.set_ylabel("出现次数")
    plt.tight_layout()
    fig.savefig(FIG_DIR / "entity_length_distribution_peoples_daily.png", dpi=120)
    print(f"  已保存 → {FIG_DIR / 'entity_length_distribution_peoples_daily.png'}")
    plt.close()
...
def main():
    ...
    print("下一步：python src_llm/train_sft.py  或 自己写训练脚本")
