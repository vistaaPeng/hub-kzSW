...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            ...
            y_pred = torch.argmax(y_pred, dim=1)  # 取出概率值最大的类别作为预测结果
...
# 用来测试每轮模型的准确率
def evaluate(model):
    model.eval() # 告诉模型现在在评估的过程中
    test_sample_num = 100
    x, y = build_dataset(test_sample_num)
    print("本次预测集中共有%d个0类样本，%d个1类样本，%d个2类样本，%d个3类样本，%d个4类样本" % (sum(y == 0), sum(y == 1), sum(y == 2), sum(y == 3), sum(y == 4)))
    correct, wrong = 0, 0
    with torch.no_grad():  # 不计算梯度， 已经开始预测了，不是训练的过程
...
def main():
    ...
    #print("训练集真实值：", train_y)
    # 训练过程
    for epoch in range(epoch_num):
        model.train() # 告诉模型现在要开始训练了
        watch_loss = []
...
def predict(model_path, input_vec):
    ...
    with torch.no_grad():
        ...
        raw_output = model.linear(x)
        probabilities = model.activation(raw_output, dim=1)
        result = torch.argmax(probabilities, dim=1)
    for vec, res, prob in zip(input_vec, result, probabilities):
        print("输入：%s, 预测类别：%d, 最大概率：%f" % (vec, int(res), float(prob[res])))  # 打印结果
